...
#[derive(IntoElement)]
pub struct Switch {
    external_focus: Option<gpui::FocusHandle>,
    id: ElementId,
...
impl Switch {
    /// Use the retained host's stable focus identity.
    pub fn with_focus_handle(mut self, handle: gpui::FocusHandle) -> Self {
        self.external_focus = Some(handle);
        self
    }
    ...
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        div().refine_style(&self.style).child(
            BaseSwitch::new(self.id.clone())
                .when_some(self.external_focus, |this, handle| {
                    this.with_focus_handle(handle)
                })
                .checked(checked)
