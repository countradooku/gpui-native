...
impl Notification {
    ...
    pub fn owned_by(mut self, owner: std::rc::Weak<()>) -> Self {
...
impl NotificationList {
    ...
    pub(crate) fn prune_owners(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let stale: Vec<_> = self
            ...
            .collect();
        for note in stale {
            note.update(cx, |note, _| {
                note.owner = None;
                note.on_close = None;
            });
            self.close(note.read(cx).id.clone(), window, cx);
        }
    }
