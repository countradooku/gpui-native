...
pub enum Text {
    ...
    /// Host-owned text rendering and selection, with an accessible plain value.
    Custom(SharedString, std::rc::Rc<dyn Fn() -> gpui::AnyElement>),
}
...
impl Text {
    /// Supply a shared text-pipeline renderer without losing the accessible value.
    pub fn custom(value: SharedString, render: impl Fn() -> gpui::AnyElement + 'static) -> Self {
        Self::Custom(value, std::rc::Rc::new(render))
    }
    ...
    pub(crate) fn get_text(&self, cx: &App) -> SharedString {
        match self {
            Self::String(value) | Self::Custom(value, _) => value.clone(),
