...
impl Root {
    ...
    pub fn render_notification_layer(
        ...
        let notifications = root.read(cx).notification.clone();
        notifications.update(cx, |list, cx| list.prune_owners(window, cx));
    ...
    pub fn render_sheet_layer(
        ...
        let root = window.root::<Root>()??;
        root.update(cx, |root, cx| root.prune_owned_overlays(window, cx));
    ...
    /// Attach the newest dialog to a retained host's lifetime.
    pub fn own_last_dialog(&mut self, owner: std::rc::Weak<()>) {
        if let Some(dialog) = self.active_dialogs.last_mut() {
            dialog.owner = Some(owner);
        }
    }

    /// Attach the current sheet to a retained host's lifetime.
    pub fn own_sheet(&mut self, owner: std::rc::Weak<()>) {
        if let Some(sheet) = self.active_sheet.as_mut() {
            sheet.owner = Some(owner);
        }
    }

    /// Remove only overlays whose retained owners have gone away. Preserve the
    /// focus chain when a lower dialog is removed while another remains open.
    fn prune_owned_overlays(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let mut index = 0;
        while index < self.active_dialogs.len() {
            if self.active_dialogs[index]
                .owner
                .as_ref()
                .is_some_and(|owner| owner.upgrade().is_none())
            {
                ...
                if let Some(next) = self.active_dialogs.get_mut(index) {
                    next.previous_focused_handle = removed.previous_focused_handle;
                } else if let Some(handle) =
                    removed.previous_focused_handle.and_then(|h| h.upgrade())
                {
