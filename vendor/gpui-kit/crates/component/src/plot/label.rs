...
impl PlotLabel {
    ...
    pub fn paint(&self, bounds: &Bounds<Pixels>, window: &mut Window, cx: &mut App) {
        ...
        } in self.0.iter()
        {
            ...
            let painter = LABEL_PAINTER.with(|slot| slot.borrow().clone());
            if let Some(painter) = painter {
                painter(text.clone(), &line, origin, *font_size, *align, window, cx);
            } else {
...
/// An embedding renderer may route plot labels through its shared document text pipeline.
pub type LabelPainter = std::rc::Rc<
    dyn Fn(SharedString, &ShapedLine, Point<Pixels>, Pixels, TextAlign, &mut Window, &mut App),
>;
thread_local! {static LABEL_PAINTER: std::cell::RefCell<Option<LabelPainter>> = const { std::cell::RefCell::new(None) };}
/// Scope a label painter to one plot's paint operation, restoring nested callers.
pub fn with_label_painter<R>(painter: LabelPainter, f: impl FnOnce() -> R) -> R {
    struct Restore(Option<LabelPainter>);
    impl Drop for Restore {
        fn drop(&mut self) {
            LABEL_PAINTER.with(|slot| *slot.borrow_mut() = self.0.take());
        }
    }
    let _restore = Restore(LABEL_PAINTER.with(|slot| slot.replace(Some(painter))));
    f()
}
