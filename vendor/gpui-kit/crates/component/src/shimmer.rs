...
///     .spread(0.4)
/// ```
pub type TextPaintObserver = std::rc::Rc<dyn Fn(gpui::TextLayout, &mut Window)>;

#[derive(IntoElement)]
pub struct ShimmerText {
    text: SharedString,
    observer: Option<TextPaintObserver>,
...
impl ShimmerText {
    ...
    pub fn observe_paint(mut self, observer: TextPaintObserver) -> Self {
...
impl RenderOnce for ShimmerText {
    fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        if cx.reduce_motion() && self.observer.is_none() {
            return container
...
impl Element for ShimmerGlyphs {
    ...
    fn paint(
        ...
        if let Some(observer) = &self.observer {
            observer(self.text.layout().clone(), window);
        }
        self.text.paint(
            global_id,
            inspector_id,
