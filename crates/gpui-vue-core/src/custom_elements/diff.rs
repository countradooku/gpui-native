...
fn show_more_row(remaining: u32, ix: usize, ctx: &RowContext, theme: &Theme) -> gpui::AnyElement {
    ...
    let mut row = gpui::div()
        .id(SharedString::from(format!("__gpui_vue_diff_more_{ix}")))
        .w_full()
...
fn file_header_row(
    ...
    let mut header = gpui::div()
        .id(SharedString::from(format!("__gpui_vue_diff_hdr_{ix}")))
        .w_full()
...
fn diff_line_row(
    ...
    let mut row = gpui::div()
        .id(SharedString::from(format!("__gpui_vue_diff_line_{ix}")))
        .h(px(m.diff_line_height))
        .w_full()
        .flex_none()
        .flex()
