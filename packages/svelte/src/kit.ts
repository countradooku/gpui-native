import { KIT_COMPONENTS, type KitComponentProps, type KitModelName } from "@gpui-native/runtime/kit"

import { nativePrimitive, kitBindingProps } from "./components.js"
import type { HostProps } from "./types.js"
export type * from "@gpui-native/runtime/kit"

function component<K extends keyof KitComponentProps>(name: K) {
  return nativePrimitive<
    KitComponentProps[K] & HostProps,
    KitModelName<K> & keyof (KitComponentProps[K] & HostProps) & string
  >(KIT_COMPONENTS[name], (source) => kitBindingProps(name, source))
}
